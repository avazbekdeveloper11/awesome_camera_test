...
RGBA saturation(RGBA color, num saturation) {
  ...
  final num gray = 0.2989 * color.red + 0.5870 * color.green + 0.1140 * color.blue; //weights from CCIR 601 spec
  return RGBA(
...
RGBA hueRotation(RGBA color, int degrees) {
  ...
  return RGBA(
    red: clampPixel(
        ((.299 + .701 * U + .168 * W) * r + (.587 - .587 * U + .330 * W) * g + (.114 - .114 * U - .497 * W) * b)
            .round()),
    green: clampPixel(
        ((.299 - .299 * U - .328 * W) * r + (.587 + .413 * U + .035 * W) * g + (.114 - .114 * U + .292 * W) * b)
            .round()),
    blue: clampPixel(
        ((.299 - .3 * U + 1.25 * W) * r + (.587 - .588 * U - 1.05 * W) * g + (.114 + .886 * U - .203 * W) * b).round()),
    alpha: color.alpha,
  );
}

RGBA grayscale(RGBA color) {
  final int avg = clampPixel((0.2126 * color.red + 0.7152 * color.green + 0.0722 * color.blue).round());
  return RGBA(
    red: avg,
    green: avg,
    blue: avg,
